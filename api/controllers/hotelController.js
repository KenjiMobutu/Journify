...
export const getTodaySales = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch today sales", details: err.message });
  }
};
...
export const getPreviousSales = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch previous sales", details: err.message });
  }
};
...
export const getLast6Months = async (req, res, next) => {
  try {
    ...
    for (let i = 5; i >= 0; i--) {
      const month =
        new Date(today.getFullYear(), today.getMonth() - i, 1).getMonth() + 1; // Obtenir le mois de chaque itération
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch last 6 months sales",
        details: err.message,
      });
  }
};
